...
(Adapted from https://github.com/salesforce/GeDi/blob/master/train_GeDi.py)

Train a modifier using custom data.
...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch_ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # Calculating loss here using reduction = None, as we can't edit modeling_gpt2.py anymore
            shift_logits = outputs['logits'][..., :-1, :].contiguous()
            shift_labels = seq_batched[..., 1:].contiguous()

            loss_fct = CrossEntropyLoss(reduction="none")
            losses_raw = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))

            # losses = outputs[0].view(seq_batched.shape[0], -1)
            losses = losses_raw.reshape(seq_batched.shape[0], -1)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # multi-gpu eval
        if args.n_gpu > 1 and not isinstance(model, torch.nn.DataParallel) and not isinstance(model,
                                                                                              torch.nn.parallel.DistributedDataParallel):
            model = torch.nn.DataParallel(model)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    # This parameter is no longer generated, stubbing
    all_token_type_ids = torch.zeros(all_attention_mask.size())
...
def main():
    ...
    parser.add_argument("--jigsaw_no_toxic_gen", action="store_true",
                        help="custom setup for jigsaw - gen_loss used only for non-toxic samples | check training loop")
    parser.add_argument("--code_0", type=str, default="negative",
                        help="control code to be used for code 1 of 2 (we support 3 at most - with the third one = 'neutral' for now)")
    parser.add_argument("--code_1", type=str, default="positive",
                        help="control code to be used for code 2 of 2 (we support 3 at most - with the third one = 'neutral' for now)")
